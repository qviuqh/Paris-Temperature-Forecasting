...
def replace_outliers(df, features, lower_bounds = {}, upper_bounds = {} , train = False):
    """
    ...
    for feature in features:
        ...
        #capping
        df[feature] = np.where(df[feature] < lower_bounds[feature], lower_bounds[feature], df[feature])
        df[feature] = np.where(df[feature] > upper_bounds[feature], upper_bounds[feature], df[feature])
    return df
